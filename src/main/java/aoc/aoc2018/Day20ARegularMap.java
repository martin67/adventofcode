...
public class Day20ARegularMap {
    ...
    static class DAG {
        ...
        final HashMap<Position, Node> nodeMap = new HashMap<>();
        final boolean blockDetours;
        Node start;
...
//        System.out.println("nextNodeParents: " + nextNodeParents + ", newChildren: " + newChildren);
        ...
        static class Node {
            ...
            Node(char direction, int distanceToStart, Position parent) {
                ...
                switch (direction) {
                    case 'N' -> this.position = new Position(parent.getX(), parent.getY() - 2);
                    case 'S' -> this.position = new Position(parent.getX(), parent.getY() + 2);
                    case 'E' -> this.position = new Position(parent.getX() + 2, parent.getY());
                    case 'W' -> this.position = new Position(parent.getX() - 2, parent.getY());
                }
                id = idCounter;
                idCounter++;
            }
